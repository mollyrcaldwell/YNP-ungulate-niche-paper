...
file_list <- list.files(path = "./Code Output/dBBMM_UDs_50m/season/", pattern = ".tif$",
                        full.names = TRUE)

name_list <- list.files(path = "./Code Output/dBBMM_UDs_50m/season/", pattern = ".tif$",
                        full.names = FALSE)
...
#change name of raster to cid
for(i in 1:length(bb_rast_list)){
  bb_rast_list[[i]]@data@names <- name_list[[i]]
}
...
#save data
saveRDS(sp_all, "./Code Output/dBBMM_UDs_50m/allindiv_seas_dbbm_spatpts_df.rds")
```

```{r}
#load saved data
sp_all <- readRDS("./Code output/dBBMM_UDs_50m/allindiv_seas_dbbm_spatpts_df.rds")
...
HR_olt <- kerneloverlap(sp_all[,2], method = c("BA"), percent = 95, conditional = FALSE)
...
```{r}
#calculate pairwise overlap of each HR (across all seasons and yrs)
#can set method to whichever best fits your purposes, documentation in kerneloverlap function help
HR_ol <- kerneloverlap(sp_all[,2], method = c("BA"), percent = 95, conditional = FALSE)
