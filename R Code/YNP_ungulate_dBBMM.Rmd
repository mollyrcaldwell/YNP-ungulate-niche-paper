...
library(move)
#library(devtools)
#install_version("BBMM", version = 3.0)
library(BBMM)
library(lubridate)
library(parallel)
```
...
grd <- raster(ext)
res(grd) <- 50     #i'm using a 50m resolution here. Might want to increase this if things are going slowly. Or decrease it if you want more precision
ncell(grd)     #number of grid cells
...
fldr <- "./Code Output/dBBMM_UDs_50m/season/"
...
```{r}
# verify your folder to write your BBs to is what you want!
fldr <- "./Code output/dBBMM_UDs_50m/month/"
dir(fldr)   # There shouldn't be any .tif files with dynBB... if so, you might want to delete them.
...
```{r}
# verify your folder to write your BBs to is what you want!
fldr <- "./Code output/dBBMM_UDs_50m/week/"
dir(fldr)   # There shouldn't be any .tif files with dynBB... if so, you might want to delete them.
...
```{r}
# verify your folder to write your BBs to is what you want!
fldr <- "./Code output/dBBMM_UDs_50m/day/"
dir(fldr)   # There shouldn't be any .tif files with dynBB... if so, you might want to delete them.
